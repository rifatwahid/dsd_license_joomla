...
$attr = '';
$folder = $folder ?: '';
$folder = 'smartshoplocal-components/com_jshopping/files/:/' . $folder;
...
$view = (isset($view) && $view) ? $view : 'images';
$authorId = (isset($type) && $type) ? $type : 'smartshopimgs';
$preview = (isset($preview) && $preview) ? $preview : 'none';
$previewWidth = $previewWidth ?? 300;
$previewHeight = $previewHeight ?? 300;

$mediatypeslist = [
	'smartshopimgs' => '0',
	'smartshopallfiles' => '0,1,2,3',
	'smartshopimgsvideo' => '0,2',
	'smartshopimgsvideodocs' => '0,2,3',
];
if(isset($mediaTypes) && $mediaTypes){
	$mediaTypes = $mediaTypes;
}elseif(isset($mediatypeslist[$authorId])){
	$mediaTypes = $mediatypeslist[$authorId];
}else{
	$mediaTypes = $mediatypeslist['smartshopallfiles'];
}
...
$url = ((isset($readonly) && $readonly) ? ''
	: ((isset($link) && $link) ? $link : 'index.php?option=com_media&view=media&tmpl=component&mediatypes=' . $mediaTypes
		. '&asset=' . $asset )
...
if (empty($doc->getScriptOptions('media-picker'))) {
	$mediaParams = ComponentHelper::getParams('com_media');

	$doc->addScriptOptions('media-picker', [
		'images'    => $mediaParams->get('image_extensions', 'bmp,gif,jpg,jpeg,png,webp'),
		'audios'    => $mediaParams->get('audio_extensions', 'mp3,m4a,mp4a,ogg'),
		'videos'    => $mediaParams->get('video_extensions', 'mp4,mp4v,mpeg,mov,webm'),
		'documents' => $mediaParams->get('doc_extensions', 'doc,odg,odp,ods,odt,pdf,ppt,txt,xcf,xls,csv'),
	]);
}
...
<joomla-field-media class="field-media-wrapper" type="image" <?php // @TODO add this attribute to the field in order to use it for all media types
																															?> base-path="<?php echo Uri::root(); ?>" root-folder="components/com_jshopping/files" url="<?php echo $url; ?>" modal-container=".modal" modal-width="100%" modal-height="400px" input=".field-media-input" button-select=".button-select" button-clear=".button-clear" button-save-selected=".button-save-selected" preview="static" preview-container=".field-media-preview" preview-width="<?php echo $previewWidth; ?>" preview-height="<?php echo $previewHeight; ?>" supported-extensions="<?php echo str_replace('"', '&quot;', json_encode(['images' => $imagesAllowedExt, 'audios' => $audiosAllowedExt, 'videos' => $videosAllowedExt, 'documents' => $documentsAllowedExt])); ?>">
	<?php echo $modalHTML; ?>
	<?php if ($showPreview) : ?>
