...
<?php foreach($rows as $row){?>
  <tr class="row<?php echo $i % 2;?>">
   ...
   <td>
    ...
    <?php if (defined(JText::_('COM_SMARTSHOP_STPAGE_'.$row->alias))) print constant(JText::_('COM_SMARTSHOP_STPAGE_'.$row->alias)); else print $row->alias;?>
    </a>
   </td>
   <td class="center">
   ...
   <?php if (!in_array($row->alias, $jshopConfig->sys_static_text)){?>
    <a class="btn btn-micro" href='index.php?option=com_jshopping&controller=config&task=deletestatictext&id=<?php print $row->id?>' onclick="return confirm('<?php print JText::_('COM_SMARTSHOP_DELETE')?>')">
