...
class jshopCart
{
    ...
    */
    ...
    // TODO: Delete $upload_file_product = '' and correct code!!!
    function add($product_id, $quantity, $attr_id, $freeattributes, $additional_fields = array(), $usetriggers = 1, &$errors = array(), $displayErrorMessage = 1, $uploadDataArr = []){
        $currentObj = $this;
        $jshopConfig = JSFactory::getConfig();
        if ($quantity <= 0){
        ...
        if ($usetriggers){
            $dispatcher = \JFactory::getApplication();
            ExcludeAttributeForAttribute::getInstance()->onBeforeAddProductToCart(&$currentObj, &$product_id, &$quantity, &$attr_id, &$freeattributes, &$updateqty, &$errors, &$displayErrorMessage, &$additional_fields, &$usetriggers);
        ...
        if ($updateqty){
        foreach ($this->products as $key => $value){
            if ($value['product_id'] == $product_id && $value['attributes'] == $attr_serialize && $value['freeattributes']==$free_attr_serialize && $value['upload_file_product']==$upload_file_product){
        ...
        if ($new_product){
            ...
            $temp_product['is_upload_independ_from_qty'] = $product->is_upload_independ_from_qty;
    ...
    function refresh($quantity){
        ...
        CheckoutMinMaxQuantityMambot::getInstance()->onBeforeRefreshProductInCart(&$quantity, &$currentObj);
        ...
        if (is_array($quantity) && count($quantity)){
            ...
            foreach($quantity as $key=>$value){
                ...
                if (!$product->unlimited && $jshopConfig->controler_buy_qty && ($checkqty > $qtyInStock)){
					...
					throw new Exception(JText::sprintf('COM_SMARTSHOP_ERROR_EXIST_QTY_PRODUCT_BASKET', $product->$name, $qtyInStock),105);	
                    continue;
                }
