...
class jshopCart
{
    ...
    */
    ...
    function getTax($incShiping = 0, $incRabatt = 0, $incPayment = 0){
		...
		$getRoundPriceProduct=getRoundPriceProduct($tax_summ);
		$dispatcher = \JFactory::getApplication();
        $dispatcher->triggerEvent('onAfterCartGetTax', array(&getRoundPriceProduct));
    return $getRoundPriceProduct;
    }
    ...
    function getTaxExtCalcAfterDiscount($incShiping = 0, $incPayment = 0){
		...
		$currentObj = $this;
		$dispatcher = \JFactory::getApplication();
        $dispatcher->triggerEvent('onBeforeCartGetTaxExtCalcAfterDiscountBefore', array(&$currentObj));
        foreach($this->products as $key => $value) {
			$dispatcher->triggerEvent('onBeforeCartGetTaxExtCalcAfterDiscount', array(&$currentObj, &$summ, $value));
		...
		$dispatcher->triggerEvent('onBeforeCartGetTaxExtCalcAfterDiscountAfter', array(&$currentObj));
    ...
    // TODO: DELETE params ($upload_file_product, $upload_file_description) after test!
    function add($product_id, $quantity, $attr_id, $freeattributes, $additional_fields = array(), $usetriggers = 1, &$errors = array(), $displayErrorMessage = 1, $upload_file_product = '', $upload_file_description = ''){
        $currentObj = true;
        $jshopConfig = JSFactory::getConfig();
		$c_params     = JComponentHelper::getParams('com_jshopping');
        $tax_source   = $c_params->get('tax_source', 'default');
        if ($quantity <= 0){
        ...
        if ($updateqty){
        foreach ($this->products as $key => $value){
            // TODO: Delete and correct $upload_file_product
        ...
        if (is_array($quantity) && count($quantity)){
            ...
            foreach($quantity as $key=>$value){
                ...
                if (!$product->unlimited && $jshopConfig->controler_buy_qty && ($checkqty > $qtyInStock)){
                    //JError::raiseWarning(113, JText::sprintf('COM_SMARTSHOP_ERROR_EXIST_QTY_PRODUCT_BASKET', $product->$name, $qtyInStock));
					throw new Exception(JText::sprintf('COM_SMARTSHOP_ERROR_EXIST_QTY_PRODUCT_BASKET', $product->$name, $qtyInStock),113);	
                    continue;
                }
    ...
    function reloadRabatValue(){
        ...
        if ($this->rabatt_type == 1){
            ...
            $this->rabatt_summ = $this->getSummForCalculeDiscount() > 0 ? $this->rabatt_value / 100 * $this->getSummForCalculeDiscount() : $this->rabatt_value; //percent
        }
