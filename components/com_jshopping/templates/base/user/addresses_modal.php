...
<div class="shop container-popup addressPopup">
	<form action="/index.php?option=com_jshopping&controller=user&task=addressPopup&limitstart=<?php echo $limitStart; ?>" method="post" id="adminForm" name="adminForm" class="addressModal">
		...
		<button id="addressPopupAddNewAddress" onClick="open('<?php echo SEFLink("index.php?option=com_jshopping&controller=user&task=addNewAddress&isCloseTabAfterSave=1", 1); ?>', '_blank', '');" class="user-addresses__new mb-3 pt-3 pb-3 ps-5 btn btn-link ps-0">
		...
		<?php else : ?>
			...
			<div class="addressPopup__tbody">
				<?php foreach ($addresses as $i => $userAddress) : ?>
					<div class="user-address border border-secondary mb-3 pt-2 pb-2 ps-5 pe-3 row<?php echo $i % 2; ?> addressPopup__address" data-address-id="<?php echo $userAddress->address_id; ?>" onclick="parent.shopUserAddressesPopup.runAddressHandler(this);">
