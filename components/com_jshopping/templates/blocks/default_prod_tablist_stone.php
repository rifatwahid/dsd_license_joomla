	...
	<div id="additional-information" role="tabpanel" class="tab-pane fade" aria-labelledby="nadditional-information">
		<ul class="list-unstyled">
			...
			<?php if ($this->config->product_show_manufacturer && $this->product->manufacturer_info->name != '') : ?>
				<li> <?php echo JText::_('COM_SMARTSHOP_MANUFACTURER'); ?>:
					<a href="<?php echo SEFLink('index.php?option=com_jshopping&controller=manufacturer&task=view&manufacturer_id=' . $this->product->product_manufacturer_id, 2); ?>">
						<?php echo $this->product->manufacturer_info->name; ?>
					</a>
				</li>
			<?php endif; ?>
			...
			<?php if ($this->product->product_basic_price_show) { ?>
				<li><?php echo JText::_('COM_SMARTSHOP_BASIC_PRICE'); ?>: <?php echo precisionformatprice(isset($this->product->product_basic_price_calculate) ? $this->product->product_basic_price_calculate : 0); ?> / <?php echo isset($this->product->product_basic_price_unit_name) ? $this->product->product_basic_price_unit_name : ''; ?>
			<?php } ?>
			...
			<?php if (!empty($this->demofiles)) {
				include __DIR__ . '/demofiles_stone.php';
			} ?>
		</ul>
	</div>
