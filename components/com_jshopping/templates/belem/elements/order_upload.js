...
const Order_upload = (data) => {
    ...
    let unserialize_prod = <Unserialize sdata={data.prod.buttons} link={data.jdata.sereliaze_link}/>;
    if (unserialize_prod['upload'] == 1 ){style= "display: none;";}

    const remainingQtyUpload = (data.prod.quantity - sumOfQtyUpload) ? data.prod.quantity - sumOfQtyUpload : 0;
        ...
        {(isMultiUpload != 0) ?
            <div className="nativeMultiuploadProgressHeader">
                <div className="row">
                    ...
                    <div className="col-md-6 align-self-center">
                        <div className="nativeMultiuploadProgressHeader__newUpload">
                            <a href="#" className="nativeMultiuploadProgressHeader__newUploadLink" onClick={(e) => uploadImage.addNewUploadOrder('.nativeProgressUploads--'+uploadBlockNumber, e, 1)}>
                ...
                : ''}
                    ...
                    <input type="hidden" name="nativeProgressUpload_allow_files_size" id="nativeProgressUpload_allow_files_size" value={data.jdata.upload_common_settings.allow_files_size} />
            </div>
            : '' }

        <div className="row nativeProgressUploads__rows">
            {(isSupportUpload == true && result) ?
                ...
                <div className="col-md-12 mb-4" data-native-upload-row-number={key} key={key} >
                    <div className="nativeProgressUpload nativeProgressUpload--nouploaded"  onClick={uploadIm}>
                        ...
                        <div className={(files == 0 || files == '') ? 'nativeProgressUpload-imageInfo display--none' : 'nativeProgressUpload-imageInfo'}>
                                ...
                                <div className="col-md-8 align-self-center">
                                    <div className={(isMultiUpload == null) ? 'nativeProgressUpload-imageInfo__qty  display--none' : 'nativeProgressUpload-imageInfo__qty'}>
                                        <input type="number" className="nativeProgressUpload-imageInfo__qtyInput"
                                               name={"nativeProgressUpload[uploads]["+uploadBlockNumber+"][qty][]"} min="0"
                                               onChange={(e) => shopCart.updateUploadImageQuantity(uploadBlockNumber)}
                                               defaultValue={(typeof data.prod.uploadData['qty'] != 'undefined' && typeof data.prod.uploadData['qty'][key] != 'undefined' && data.prod.uploadData['qty'][key]) ? data.prod.uploadData['qty'][key] : 1} />
                            ...
                            {(data.jdata.upload_common_settings.upload_design == 1) ?
                                <Form.Control type="number" name={"nativeProgressUpload[uploads]["+uploadBlockNumber+"][descriptions][]"} className="nativeProgressUpload-imageInfo__deajaxUpdateDescribeUploadImageInCartscribeInput" onChange={(e) => ajaxUpdateDescribeUploadImageInCart(uploadBlockNumber)} defaultValue={(typeof data.prod.uploadData != 'undefined' && data.prod.uploadData['descriptions'][key] != 'undefined') ? data.prod.uploadData['descriptions'][key] : ''} placeholder={Joomla.JText._('COM_SMARTSHOP_UPLOAD_DESCRIPTION')} />
                            : ''}

                            <input type="hidden" name={"nativeProgressUpload[uploads]["+uploadBlockNumber+"][previews][]"} value={(typeof data.prod.uploadData != 'undefined'&& typeof data.prod.uploadData['previews'] != undefined && typeof data.prod.uploadData['previews'] != 'undefined' && typeof data.prod.uploadData[key] != undefined  && typeof data.prod.uploadData['previews'][key] != undefined && data.prod.uploadData['previews'][key] != null) ? "/components/com_jshopping/files/files_upload/" + data.prod.uploadData['previews'][key] : "/components/com_jshopping/files/files_upload/"} className="nativeProgressUpload__imageInput"/>
                            <input type="hidden" name={"nativeProgressUpload[uploads]["+uploadBlockNumber+"][files][]"} value={(files != '' && files != 0) ? files : ''} className="nativeProgressUpload__fileInput"/>
                        </div>
                    </div>
                    <div dangerouslySetInnerHTML={{ __html: data.prod.js_template_for_native_uploaded }} />
                    {/*{Parser(data.prod.js_template_for_native_uploaded)}*/}
                </div>
             )
             : ''}
